...
export async function signInWithWallet({ supabase, method, onStatus }) {
  ...
  onStatus?.(`Switching your wallet to ${CHAIN.name}`);
  await ensureChain(eth);                                    // adds Base if the wallet has never seen it

  // Wallet already linked to an email Core? Sign in to THAT Core rather than minting a wallet-only one.
  const probe = await fetch(`/api/auth/wallet-login?address=${checksummed}`).then(r => r.json()).catch(() => ({}));
  if (probe?.linked && probe.message) {
    onStatus?.('Confirm the sign-in in your wallet (free, no transaction)');
    const sig = await eth.request({ method: 'personal_sign', params: [probe.message, address] });
    const r = await fetch('/api/auth/wallet-login', { method: 'POST', headers: { 'content-type': 'application/json' }, body: JSON.stringify({ address: checksummed, signature: sig, method }) });
    const d = await r.json();
  ...
  if (error) {
    ...
    if (/saving new user/i.test(m)) throw new Error('Could not create your Core just now — try again in a moment.');
    if (/rate limit/i.test(m)) throw new Error('Too many sign-ins from here right now — wait a few minutes.');
    if (/rejected|denied|4001/i.test(m)) throw new Error('Sign-in cancelled in the wallet.');
    throw new Error(m || 'sign-in failed');
  }
