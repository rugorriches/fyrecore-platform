...
const posts = [
  { d:'2026-09-09', t:'Two sections, and why', b:'OMEN will not whitelist a contract we own on OMENX. So anything sold there has to live on their contracts, in their currencies, under their rules. Rather than blur that, the site now has two sections. FyreCore titles run on rails we own, are priced in dollars, and take card, USDC, OMENX or GMT at the same price. OMEN titles run on OMEN, full stop. The database enforces it: an OMEN-section game cannot accept dollars and an OMEN-tier collection cannot be marked as ours. We are an independent developer there and the page says so.' },
  { d:'2026-09-08', t:'We cut the token', b:'FYRE is gone. Not paused, not deferred to a later phase, cut. The design was careful: fixed supply, no presale, no team allocation, unlock gated on real play. It would still have been the riskiest and most expensive thing here, needing two audits, token counsel and launch liquidity before a single match was played. Against a two-person studio that money buys a finished game instead. Prize money is USDC. Passes are a card payment. The Vault floor, the escrowed brackets, the public ledger and the earned-resale threshold all survive intact, because none of them ever needed a currency of our own. They needed a unit of account, and the dollar is one.' },  { d:'2026-09-07', t:'A production environment variable that broke every build', b:'The game repo would not deploy. The cause was not the code: NODE_ENV was set to production as a system variable on the dev machine, which makes npm omit devDependencies in every project on that machine. npm ci installed 2 packages instead of 66, so the TypeScript compiler and the bundler simply were not there. Worth knowing if your builds fail in ways that make no sense.' },
